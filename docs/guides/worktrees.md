...
- For a structured view (ACP) session, the worktree move resets the agent conversation: the agent's prior session was rooted at the old directory, so resuming it would point the agent at a path that no longer exists. After the rename the worker is torn down and the next prompt starts a fresh agent session at the new directory.
